...
function PreviewImages({ limit }) {
  ...
  return (
    <>
      <div className="grid w-[360px] min-h-[100px] gap-2 grid-cols-3 grid-row-3 overflow-hidden max-h-[350px] overflow-y-scroll">
        {images?.length <= 0 && (
        ...
        {images?.map((img, inx) => {
          if (inx < limit) {
            if (!img.file_url) {
              return (
                <div className="w-full h-[100px] border border-light-black rounded-lg relative group cursor-pointer flex items-center text-2xl text-light-black font-semibold justify-center">
                  {img?.progress < 100 ? (
                ...
                >
                  <div onClick={() => deleteImage(inx)}>
                    ...
                    <Img
                      ...
                      className="z-20 object-contain w-full h-[100px] rounded-lg group-hover:brightness-50"
                      alt="img"
                      src={img?.file_url}
                    />
                  </div>
